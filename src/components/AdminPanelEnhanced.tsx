...
import { supabase } from '../hooks/useSupabaseRealtime';
...
const AdminPanelEnhanced: React.FC<AdminPanelEnhancedProps> = ({
  ...
  const fetchAllData = async () => {
    setIsLoading(true);
    try {
      const [logs, banned, muted, stats, messages, users] = await Promise.all([
        adminApi.getActivityLogs(),
        adminApi.getBannedUsers(),
        adminApi.getMutedUsers(),
        adminApi.getStreamStats(),
        adminApi.getChatMessages(100),
        adminApi.getConnectedUsers()
      ]);

      if (logs.success) setActivityLogs(logs.logs || []);
      if (banned.success) setBannedUsers(banned.users || []);
      if (muted.success) setMutedUsers(muted.users || []);
      if (stats.success) setStreamStats(stats.stats || {});
      if (messages.success) setAllChatMessages(messages.messages || []);
      if (users.success) setRealConnectedUsers(users.users || []);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const messagesChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'chat_messages' }, (payload) => {
        if (payload.eventType === 'INSERT') {
          setAllChatMessages(prev => [...prev, payload.new]);
        } else if (payload.eventType === 'DELETE') {
          setAllChatMessages(prev => prev.filter(m => m.id !== payload.old.id));
        }
      })
      .subscribe();
    ...
    const bansChannel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'banned_users' }, () => {
        adminApi.getBannedUsers().then(data => {
          if (data.success) setBannedUsers(data.users || []);
        });
      })
      .subscribe();
    ...
    const mutesChannel = supabase
      .channel('muted_users_changes')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'muted_users' }, () => {
        adminApi.getMutedUsers().then(data => {
          if (data.success) setMutedUsers(data.users || []);
        });
      })
      .subscribe();
  ...
  const handleBanUser = async () => {
    ...
    try {
      await adminApi.banUser(
        ...
        selectedUser.ip_address || selectedUser.ip || '',
      ...
      // Déconnecter l'utilisateur via WebSocket
      if (wsService) {
        wsService.send({
  ...
  const handleUnban = async (bannedUser: any) => {
    try {
      await adminApi.unbanUser(bannedUser.id, currentUser.username);
    } catch (error) {
      console.error('Error unbanning user:', error);
    }
  };

  const handleUnmute = async (mutedUser: any) => {
    try {
      await adminApi.unmuteUser(mutedUser.id, currentUser.username);
    } catch (error) {
  ...
  const handleDeleteMessage = async (messageId: string) => {
    try {
      await adminApi.deleteMessage(messageId, currentUser.username);
      if (wsService) {
        wsService.send({ type: 'message_deleted', messageId });
      }
    } catch (error) {
      ...
      <div className="max-w-[1800px] mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400">Données en temps réel depuis Supabase</p>
          </div>
          <div className="flex items-center gap-3">
            <button
              onClick={() => setAutoRefresh(!autoRefresh)}
              className={`p-3 rounded-xl transition-all ${
                autoRefresh ? 'bg-green-500/20 text-green-400' : 'bg-slate-800'
              }`}
            >
            ...
            <button
              ...
              disabled={isLoading}
              className="px-6 py-3 bg-cyan-500 hover:bg-cyan-600 rounded-xl transition-all disabled:opacity-50"
            >
              Rafraîchir
            </button>
          </div>
              ...
              {(realConnectedUsers.length > 0 ? realConnectedUsers : connectedUsers).map((user: any) => (
                <div key={user.id} className="p-4 bg-slate-900/50 rounded-xl flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-slate-400">{user.ip_address || user.ip} • {user.page}</div>
                  </div>
                  <div className="flex gap-2">
                    <button
                      ...
                      className="p-2 bg-orange-500/20 hover:bg-orange-500/30 text-orange-400 rounded-lg"
                    >
            ...
            <div className="space-y-3 max-h-[600px] overflow-y-auto">
              {filteredLogs.map((log) => (
                <div key={log.id} className="p-4 bg-slate-900/50 rounded-xl">
                  ...
                  <div className="text-sm text-slate-400">
                    {log.username} • {log.admin_username ? `Par: ${log.admin_username}` : ''}
                  </div>
                </div>
              ))}
            </div>
